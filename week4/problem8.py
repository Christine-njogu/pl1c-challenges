...
def main():
    ...
    user_text = input("Enter a text paragraph copied from any website: ")
    marks = string.punctuation
    for items in user_text:
        if items in marks:
            user_text = user_text.replace(items, "")

    user_text1 = user_text.lower()
    user_list = user_text1.split()
    print(user_list, len(user_list))
    print("Uncommon words:")
    for items in user_list:
        if items not in common_words:
            uncommon_list = items.split('\n')
            print(uncommon_list)
    print("Common words: ")
    for items in user_list:
        if items in common_words:
            common_list = items.split("\n")
            print(f"{common_list}; {user_list.count(items)}\trel freq: {(user_list.count(items))/len(user_list)}")
