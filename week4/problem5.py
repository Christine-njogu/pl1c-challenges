...
def main():
    ...
    Fulvous 	#E48400 
    Fuzzy Wuzzy 	#87421F"""
    colours2 = colours.replace(" ", "")
    colours3 = colours2.replace("\n", " ")
    colours_list = colours3.split()
    hex_code_list = colours_list[1:len(colours_list):2]
    colour_name_list = colours_list[:len(colours_list):2]
    colours_dict = dict(zip(hex_code_list, colour_name_list))
    user_colour = input("Enter letter 'r' or 'g' or 'b': ")
    for k in iter(colours_dict):
        if user_colour == 'r' and k[1:3].find('00') == -1:
            print(f"Red channel: {k}")
        elif user_colour == 'g' and k[3:5].find('00') == -1:
            print(f"Green channel: {k}")
        elif user_colour == 'b' and k[5:].find('00') == -1:
            print(f"Blue channel: {k}")
